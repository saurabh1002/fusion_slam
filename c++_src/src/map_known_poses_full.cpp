...
open3d::camera::PinholeCameraIntrinsic get_camera_intrinsics(
    ...
    return open3d::camera::PinholeCameraIntrinsic(width, height, fx, fy, cx,
                                                  cy);
}
...
int main(int argc, char* argv[]) {
    ...
    auto rgb_camera_intrinsic = get_camera_intrinsics(config);

    auto depth_scale = config["DepthCamera"]["depth_scale"].as<int>();
    auto depth_max = config["DepthCamera"]["max_range"].as<double>();
    ...
    for (std::size_t idx = 0; idx < dataset.size(); idx++) {
        ...
        auto rgbdImage = o3d::geometry::RGBDImage::CreateFromColorAndDepth(
                rgbImage, depthImage, depth_scale, depth_max, false);

        auto extrinsics = TF_from_poses(pose);
        tsdf_volume.Integrate(*rgbdImage, rgb_camera_intrinsic,
                              extrinsics.inverse());
    }
