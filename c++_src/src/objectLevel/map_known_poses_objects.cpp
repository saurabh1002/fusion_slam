...
int main(int argc, char* argv[]) {
    ...
    std::map<std::string, o3d::pipelines::integration::ScalableTSDFVolume>
            object_tsdf_dict{};
    ...
    for (auto& [label, volume] : object_tsdf_dict) {
        auto mesh = volume.ExtractTriangleMesh();
        mesh->ComputeVertexNormals();
        std::string filename = "../results/mesh_known_poses_" + label + ".ply";
        o3d::io::WriteTriangleMeshToPLY(filename, *mesh, false, false, true,
                                        true, true, true);
        std::cout << "Saved mesh file for " << label << std::endl;
